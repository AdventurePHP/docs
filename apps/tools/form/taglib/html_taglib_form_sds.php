<?php
   ...
   */
   class html_taglib_form extends ui_element
   {
      ...
      *  Version 0.1, 05.01.2007<br />
      */
      function addFormElementAtPlaceholder($Type, $Placeholdername){
         ...
         if(class_exists($Type)){
            ...
            $platzhalter = $this->getFormElementByName($Placeholdername);
		
			$oph =  ( $platzhalter->get('ObjectID'));
			$Content_org = $this->get('Content');
	
			$string = split($oph, $Content_org);
			$stringneu = str_replace("<".$oph." />","<".$ObjectID." />"."<".$oph." />",$Content_org);

 
            // XML-Merker-Tag im Content hinzufügen
            $this->__Content = $stringneu;
      ...
      *  Version 0.1, 05.01.2007<br />
      */
      function addFormContentAtPlaceholder($Content, $Placeholdername){
      	    $platzhalter = $this->getFormElementByName($Placeholdername);
			...
			$string = split($oph, $Content_org);
			$stringneu = str_replace("<".$oph." />",$Content."<".$oph." />",$Content_org);
